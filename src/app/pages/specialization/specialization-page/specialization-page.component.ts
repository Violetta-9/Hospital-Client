...
export class SpecializationPageComponent extends EntityDetailsBaseComponent implements OnInit {
  ...
  protected saveInternal(): any {
    ...
    this.specializationService.updateSpecialization(this.detailsForm.getRawValue()).subscribe(x => {
      if (x.isSuccess) {
        this.alertService.showSuccess('RESPONSE.SPECIALIZATION.SUCCESSFULLY_UPDATE');
      }
    });
  ...
  updateStatus($event: MatRadioChange) {
    ...
    this.specializationService.updateSpecializationStatus({
      ...
      if (x.isSuccess) {
        this.alertService.showSuccess('RESPONSE.SPECIALIZATION.SUCCESSFULLY_UPDATE_STATUS');
      }
    })
  }
